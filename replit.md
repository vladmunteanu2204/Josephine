...
- **Media Storage:** Replit Object Storage integration for persistent media file hosting. Admin can upload wallpaper (hero images), photos, and videos directly from local files via upload buttons in Trail Manager. Backend uses `replit.object-storage` Python SDK with auto-authentication. Upload endpoint (`/api/admin/upload/media`) accepts file uploads and stores them with unique UUIDs organized by type (wallpaper/photos/videos). Media files are served via `/api/media/{filename}` endpoint. Supports multiple file uploads for photo/video galleries. Cost: ~$0.03/GB/month storage + $0.10/GB data transfer. Files persist after deployment and are backed by Google Cloud Storage for high availability.
