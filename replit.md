...
- **User Authentication:** Firebase Authentication (email/password, Google OAuth) is integrated for secure user management, including profile editing and password changes. Google Sign-In features mobile-optimized redirect flow with automatic device detection (popup for desktop, redirect for mobile), intelligent fallback strategy (popup-to-redirect if blocked), coordinated loading state management, and comprehensive user-friendly error messaging for all authentication failure scenarios. Mobile testing requires opening app URL in new browser tab (not Replit iframe) for proper OAuth redirect flow.
