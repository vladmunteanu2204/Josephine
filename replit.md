...
- **Mobile Header & Hero (Phase 17H):** Mobile-first header redesign with LanguageBottomSheet (slide-up language selector with portal rendering, backdrop blur, and full accessibility) and HamburgerMenu (slide-in navigation drawer with user/admin sections). Responsive breakpoints hide desktop nav on ≤1024px, show hamburger instead. Hero section enhanced with scroll cue animation, stacked CTA buttons on mobile, responsive typography (32px-72px), and microinteractions (ripple effects, button press feedback). All components use portal rendering with proper dialog semantics (role="dialog", aria-modal, Escape key handlers, auto-focus). Desktop-only elements (auth buttons, user name) properly visible on >1024px screens. **Mobile UI fixes (iPhone 390×844):** Scroll cue on Home page now uses `bottom: calc(env(safe-area-inset-bottom, 0px) + 20px)` for proper iPhone notch spacing with centered positioning. GPS stats panel conditionally renders only when `isTracking === true`, appearing immediately at tracking start (showing "0.0km" initially). Global CSS in index.css prevents header gap by forcing `margin-top: 0 !important` on main/hero sections and setting header to `position: sticky; margin: 0; overflow: visible;` to prevent margin collapsing.
