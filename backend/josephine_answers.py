...
ANSWERS = {
    ...
    'emergency': {
        'en': [
            "Alpine rescue in Italy: call 118 (Soccorso Alpino) — the call is free, but the rescue itself, especially a helicopter, can be charged here in South Tyrol, so carry insurance or an alpine-club membership that covers it. If you can, give them your GPS coordinates — on iPhone open Maps, long-press the screen, the coordinates appear at the top. The mountain distress signal is 6 whistle blasts or torch flashes per minute. If a storm catches you on an exposed ridge: descend immediately, crouch away from the high point, avoid lone trees and summit crosses. Always tell someone your plan before heading out.",
            "In an emergency call 118 — that's mountain rescue (Soccorso Alpino); the call is free, though the rescue itself can be charged here, so be insured. The Europe-wide 112 works too and can locate you. Install the GeoResQ or 'Where ARE U' app beforehand so it can send your exact position. Six signals a minute (whistle, light or shout) is the recognised call for help; three a minute is the reply. Caught by lightning on a ridge? Get down fast, ditch metal poles, and crouch low on your pack away from the summit cross. And always leave your route and return time with someone in the valley.",
        ],
        'it': [
            "Soccorso alpino in Italia: chiama il 118 (Soccorso Alpino) — la chiamata è gratuita, ma il soccorso in sé, soprattutto in elicottero, qui in Alto Adige può esserti addebitato, quindi porta un'assicurazione o una tessera di un club alpino che lo copra. Se puoi, fornisci le tue coordinate GPS — su iPhone apri Mappe, tieni premuto sullo schermo e le coordinate compaiono in alto. Il segnale di soccorso alpino è 6 fischi o lampi di torcia al minuto. Se un temporale ti coglie su una cresta esposta: scendi subito, accovacciati lontano dal punto più alto, evita alberi isolati e croci di vetta. Prima di partire, di' sempre a qualcuno il tuo piano.",
            "In emergenza chiama il 118 — è il soccorso alpino (Soccorso Alpino); la chiamata è gratuita, ma il soccorso in sé qui può esserti addebitato, quindi assicurati. Funziona anche il 112 europeo, che può localizzarti. Installa prima l'app GeoResQ o «Where ARE U» così può inviare la tua posizione esatta. Sei segnali al minuto (fischio, luce o grido) è la richiesta di aiuto riconosciuta; tre al minuto è la risposta. Colto da un fulmine su una cresta? Scendi in fretta, abbandona i bastoncini di metallo e accovacciati basso sullo zaino, lontano dalla croce di vetta. E lascia sempre il tuo percorso e l'orario di rientro a qualcuno a valle.",
        ],
        'de': [
            "Bergrettung in Italien: wähle 118 (Soccorso Alpino) — der Anruf ist kostenlos, aber die Rettung selbst, vor allem per Hubschrauber, kann dir hier in Südtirol berechnet werden, also hab eine Versicherung oder eine Alpenvereins-Mitgliedschaft, die das abdeckt. Wenn möglich, gib deine GPS-Koordinaten durch — auf dem iPhone Karten öffnen, lange auf den Bildschirm drücken, die Koordinaten erscheinen oben. Das alpine Notsignal sind 6 Pfiffe oder Lichtzeichen pro Minute. Wenn dich ein Gewitter auf einem exponierten Grat erwischt: sofort absteigen, abseits vom höchsten Punkt in die Hocke, einzelne Bäume und Gipfelkreuze meiden. Sag vor dem Aufbruch immer jemandem deinen Plan.",
            "Im Notfall wähle 118 — das ist die Bergrettung (Soccorso Alpino); der Anruf ist kostenlos, aber die Rettung selbst kann hier berechnet werden, sei also versichert. Auch der europaweite Notruf 112 funktioniert und kann dich orten. Installiere vorher die App GeoResQ oder „Where ARE U“, damit sie deine genaue Position senden kann. Sechs Signale pro Minute (Pfiff, Licht oder Ruf) ist der anerkannte Hilferuf; drei pro Minute die Antwort. Vom Blitz auf einem Grat überrascht? Schnell runter, Metallstöcke weg und tief auf dem Rucksack kauern, abseits vom Gipfelkreuz. Und hinterlass immer Route und Rückkehrzeit bei jemandem im Tal.",
        ],
    },
    ...
    # ══ TRIP-PLANNING KNOWLEDGE (Batch 3) ════════════════════════════════════
    # General "before you go" questions tourists ask most. Facts kept general
    # where details vary year to year (parking rules, prices) — never invented.

    'bestTime': {
        'en': ["The classic hiking season is mid-June to early October. July and August have the most reliable weather and every hut open — but also the crowds and the highest prices. My favourites are late June (wildflowers, long days) and September (golden light, quiet trails, settled weather). Snow can linger on the high routes into early June, and most huts and high trails close by early-to-mid October. Winter is a different world — snowshoeing and ski touring."],
        'it': ["La stagione classica per le escursioni va da metà giugno a inizio ottobre. Luglio e agosto hanno il tempo più stabile e tutti i rifugi aperti — ma anche la folla e i prezzi più alti. I miei periodi preferiti sono fine giugno (fioriture, giornate lunghe) e settembre (luce dorata, sentieri tranquilli, tempo stabile). A inizio giugno la neve può resistere sui percorsi alti, e la maggior parte dei rifugi e dei sentieri d'alta quota chiude tra inizio e metà ottobre. L'inverno è un altro mondo — ciaspole e scialpinismo."],
        'de': ["Die klassische Wandersaison reicht von Mitte Juni bis Anfang Oktober. Juli und August haben das stabilste Wetter und alle Hütten offen — aber auch die meisten Leute und die höchsten Preise. Meine Lieblingszeiten sind Ende Juni (Blütezeit, lange Tage) und September (goldenes Licht, ruhige Wege, beständiges Wetter). Anfang Juni kann auf den hohen Routen noch Schnee liegen, und die meisten Hütten und Höhenwege schließen Anfang bis Mitte Oktober. Der Winter ist eine andere Welt — Schneeschuh und Skitouren."],
    },
    'gettingThere': {
        'en': ["The handiest airports are Verona, Venice (Marco Polo or Treviso) and Innsbruck — each roughly 2–3 hours by car. Bolzano has a small airport with limited connections; Milan and Munich are further but well linked. By train, take the Brennero line to Bolzano or Bressanone, or — for the eastern Dolomites — Brunico on the Pustertal line, then a local bus. Once you're here, the Südtirol Guest Pass makes those buses and regional trains free."],
        'it': ["Gli aeroporti più comodi sono Verona, Venezia (Marco Polo o Treviso) e Innsbruck — ognuno a circa 2–3 ore d'auto. Bolzano ha un piccolo aeroporto con collegamenti limitati; Milano e Monaco sono più lontani ma ben collegati. In treno, prendi la linea del Brennero fino a Bolzano o Bressanone, oppure — per le Dolomiti orientali — Brunico sulla linea della Pusteria, poi un bus locale. Una volta qui, la Südtirol Guest Pass rende gratuiti quei bus e i treni regionali."],
        'de': ["Die praktischsten Flughäfen sind Verona, Venedig (Marco Polo oder Treviso) und Innsbruck — jeweils rund 2–3 Autostunden entfernt. Bozen hat einen kleinen Flughafen mit wenigen Verbindungen; Mailand und München sind weiter, aber gut angebunden. Mit dem Zug nimmst du die Brennerlinie nach Bozen oder Brixen, oder — für die östlichen Dolomiten — Bruneck an der Pustertallinie, dann einen Linienbus. Vor Ort macht die Südtirol Guest Pass diese Busse und Regionalzüge kostenlos."],
    },
    'drivingAround': {
        'en': ["A car gives the most freedom, but you don't strictly need one — South Tyrol's buses and trains are excellent. If you do drive: town centres often have a ZTL (restricted zone), Italian motorways are toll roads (no vignette needed within Italy), and mountain parking is limited and paid. Several hotspots now restrict cars in summer — Lago di Braies and the Seiser Alm have daytime access limits or require a parking reservation — so check the official site before you go, or take the shuttle or cable car."],
        'it': ["L'auto dà più libertà, ma non è indispensabile — i bus e i treni dell'Alto Adige sono ottimi. Se guidi: i centri storici hanno spesso una ZTL (zona a traffico limitato), le autostrade italiane sono a pedaggio (in Italia non serve il bollino) e i parcheggi in montagna sono limitati e a pagamento. Diverse mete molto frequentate ora limitano le auto in estate — Lago di Braies e l'Alpe di Siusi hanno limiti d'accesso diurni o richiedono la prenotazione del parcheggio — quindi controlla il sito ufficiale prima di partire, oppure prendi la navetta o la funivia."],
        'de': ["Ein Auto gibt die meiste Freiheit, ist aber nicht zwingend nötig — Südtirols Busse und Züge sind hervorragend. Wenn du fährst: Ortskerne haben oft eine ZTL (verkehrsbeschränkte Zone), italienische Autobahnen sind mautpflichtig (innerhalb Italiens keine Vignette nötig), und Bergparkplätze sind begrenzt und kostenpflichtig. Mehrere Hotspots beschränken im Sommer die Autos — Pragser Wildsee und die Seiser Alm haben tagsüber Zufahrtsbeschränkungen oder verlangen eine Parkplatzreservierung — prüf also vorab die offizielle Seite oder nimm Shuttle bzw. Seilbahn."],
    },
    'guestPass': {
        'en': ["If you're staying in registered accommodation, ask your host for the Südtirol Guest Pass — it's free and covers all regional buses and trains across South Tyrol for your whole stay, plus a few cable cars (like Ritten/Renon). Not staying overnight? The paid Mobilcard (1/3/7 days) gives the same network access, and the museumobil card adds the museums. Big cable cars usually need a separate ticket unless your area's pass includes them — worth checking locally."],
        'it': ["Se alloggi in una struttura registrata, chiedi al tuo host la Südtirol Guest Pass — è gratuita e copre tutti i bus e treni regionali dell'Alto Adige per l'intero soggiorno, più alcune funivie (come il Renon). Non pernotti? La Mobilcard a pagamento (1/3/7 giorni) dà lo stesso accesso alla rete, e la museumobil card aggiunge i musei. Le grandi funivie di solito richiedono un biglietto a parte, a meno che il pass della tua zona non le includa — meglio verificare sul posto."],
        'de': ["Wenn du in einer gemeldeten Unterkunft wohnst, frag deinen Gastgeber nach der Südtirol Guest Pass — sie ist kostenlos und gilt für alle Regionalbusse und -züge in Südtirol während deines ganzen Aufenthalts, dazu einige Seilbahnen (etwa Ritten). Keine Übernachtung? Die kostenpflichtige Mobilcard (1/3/7 Tage) bietet denselben Netzzugang, und die museumobil Card ergänzt die Museen. Große Seilbahnen brauchen meist ein eigenes Ticket, sofern der Pass deiner Region sie nicht einschließt — am besten vor Ort prüfen."],
    },
    'itinerary': {
        'en': ["For a first taste, 4–5 days lets you mix a couple of valleys with two or three signature hikes without rushing. A week is the sweet spot — room for a hut-to-hut leg, a rest or rain day, and the famous spots plus a quieter one. Base yourself in one or two valleys rather than moving every night; the driving between areas eats more time than people expect. Tell me how many days you have and I'll help shape it."],
        'it': ["Per un primo assaggio, 4–5 giorni permettono di combinare un paio di valli con due o tre escursioni simbolo senza correre. Una settimana è l'ideale — spazio per una tappa di rifugio in rifugio, un giorno di riposo o di pioggia, e i posti famosi più uno più tranquillo. Stabilisciti in una o due valli invece di spostarti ogni notte; gli spostamenti tra le zone richiedono più tempo di quanto si pensi. Dimmi quanti giorni hai e ti aiuto a organizzarli."],
        'de': ["Für einen ersten Eindruck reichen 4–5 Tage, um ein paar Täler mit zwei, drei markanten Touren ohne Hetze zu verbinden. Eine Woche ist ideal — Platz für eine Hütten-zu-Hütten-Etappe, einen Ruhe- oder Regentag und die berühmten Orte plus einen ruhigeren. Quartier dich in einem, zwei Tälern ein, statt jede Nacht umzuziehen; die Fahrten zwischen den Gebieten kosten mehr Zeit als gedacht. Sag mir, wie viele Tage du hast, und ich helfe beim Planen."],
    },
    'viaFerrata': {
        'en': ["A via ferrata (Klettersteig) is a protected climbing route — you follow a steel cable bolted to the rock, clipping in as you go. It's not regular hiking: you need a via ferrata set (harness + a lanyard with a shock absorber) and a helmet, a head for heights, and ideally some experience. New to it? Go with a mountain guide the first time — they bring the kit and the judgement. The Dolomites have everything from gentle starter routes to serious alpine lines, so match the grade to your level."],
        'it': ["Una via ferrata (Klettersteig) è un percorso attrezzato — segui un cavo d'acciaio fissato alla roccia, assicurandoti man mano. Non è una semplice escursione: servono un set da ferrata (imbrago + longe con dissipatore) e un casco, un buon rapporto con l'esposizione e, idealmente, un po' d'esperienza. È la prima volta? Falla con una guida alpina — porta materiale e capacità di giudizio. Le Dolomiti hanno di tutto, da vie facili per iniziare a itinerari alpini impegnativi, quindi scegli la difficoltà adatta al tuo livello."],
        'de': ["Eine Via Ferrata (Klettersteig) ist eine gesicherte Route — du folgst einem im Fels verankerten Stahlseil und klinkst dich unterwegs ein. Das ist keine normale Wanderung: Du brauchst ein Klettersteigset (Gurt + Bandfalldämpfer) und einen Helm, Schwindelfreiheit und idealerweise etwas Erfahrung. Neu darin? Geh das erste Mal mit einem Bergführer — er bringt Ausrüstung und Erfahrung mit. Die Dolomiten bieten alles von sanften Einstiegsrouten bis zu ernsten alpinen Linien, also wähle die Schwierigkeit passend zu deinem Niveau."],
    },
    'alpineClub': {
        'en': ["Membership of an alpine club (Italy's CAI/AVS, Germany's DAV, Austria's ÖAV) pays off fast if you're staying in huts: members get a reduced overnight rate in CAI/AVS huts across the Alps, reciprocal rights, and — importantly — included alpine-rescue insurance. You can join online before your trip. For more than a night or two in the mountains, it usually costs less than it saves."],
        'it': ["L'iscrizione a un club alpino (il CAI/AVS italiano, il DAV tedesco, l'ÖAV austriaco) si ripaga in fretta se dormi nei rifugi: i soci hanno la tariffa ridotta per il pernottamento nei rifugi CAI/AVS di tutte le Alpi, diritti di reciprocità e — importante — l'assicurazione per il soccorso alpino inclusa. Puoi iscriverti online prima del viaggio. Per più di una notte o due in montagna, di solito costa meno di quanto fa risparmiare."],
        'de': ["Die Mitgliedschaft in einem Alpenverein (italienischer CAI/AVS, deutscher DAV, österreichischer ÖAV) zahlt sich schnell aus, wenn du auf Hütten übernachtest: Mitglieder zahlen einen ermäßigten Übernachtungspreis in CAI/AVS-Hütten in den ganzen Alpen, haben Gegenrechte und — wichtig — eine inkludierte Bergrettungsversicherung. Beitreten kannst du online vor der Reise. Bei mehr als ein, zwei Nächten in den Bergen kostet sie meist weniger, als sie spart."],
    },
    'simData': {
        'en': ["With an EU SIM, roaming works at no extra cost (roam-like-at-home). From outside the EU, the cheapest options are a local prepaid SIM (TIM, Vodafone or WindTre, sold at airports and phone shops) or an eSIM you activate before you land (Airalo and similar). Coverage is good in the valleys and towns but drops on the far side of a ridge — so download your maps offline either way."],
        'it': ["Con una SIM dell'UE il roaming funziona senza costi extra (roam-like-at-home). Dall'estero (fuori UE), le opzioni più economiche sono una SIM prepagata locale (TIM, Vodafone o WindTre, in vendita in aeroporto e nei negozi di telefonia) o una eSIM da attivare prima di arrivare (Airalo e simili). La copertura è buona nelle valli e nei paesi ma cala dietro le creste — quindi scarica comunque le mappe offline."],
        'de': ["Mit einer EU-SIM funktioniert Roaming ohne Aufpreis (Roam-like-at-home). Von außerhalb der EU sind die günstigsten Optionen eine lokale Prepaid-SIM (TIM, Vodafone oder WindTre, am Flughafen und in Handyshops) oder eine eSIM, die du vor der Ankunft aktivierst (Airalo und ähnliche). Der Empfang ist in Tälern und Orten gut, fällt aber hinter einem Grat ab — lade die Karten also ohnehin offline herunter."],
    },
    'budget': {
        'en': ["As a rough guide: a bed with half-board in a rifugio runs about €60–90 per person, a hot meal at altitude €15–25, and a return cable-car ticket €20–40. Valley hotels vary widely by season — July/August and Christmas are dearest, late June and September much gentler. You can keep costs down with the Guest Pass (free transport), picnic lunches, and travelling in the shoulder season. Tell me your style and length and I'll give a closer estimate."],
        'it': ["Come riferimento di massima: un letto con mezza pensione in rifugio costa circa €60–90 a persona, un pasto caldo in quota €15–25 e un biglietto andata e ritorno in funivia €20–40. Gli hotel in valle variano molto con la stagione — luglio/agosto e Natale sono i più cari, fine giugno e settembre molto più miti. Puoi contenere i costi con la Guest Pass (trasporti gratuiti), i pranzi al sacco e viaggiando in bassa stagione. Dimmi il tuo stile e la durata e ti do una stima più precisa."],
        'de': ["Als grober Anhaltspunkt: ein Bett mit Halbpension auf einer Hütte kostet etwa €60–90 pro Person, eine warme Mahlzeit in der Höhe €15–25 und ein Seilbahn-Retourticket €20–40. Talhotels schwanken stark je nach Saison — Juli/August und Weihnachten sind am teuersten, Ende Juni und September deutlich günstiger. Sparen kannst du mit der Guest Pass (kostenloser Verkehr), Picknick-Mittagessen und Reisen in der Nebensaison. Sag mir deinen Stil und die Dauer, und ich gebe eine genauere Schätzung."],
    },
    'insurance': {
        'en': ["Worth knowing: the emergency call (112/118) is free, but the rescue itself — especially a helicopter — can be billed to you here in South Tyrol; it's not covered by the Italian health service for visitors. So travel insurance that includes mountain rescue and helicopter evacuation is strongly recommended. An alpine-club membership (CAI/AVS/DAV/ÖAV) includes rescue cover, and there's also a low-cost seasonal scheme (Aiut Alpin Dolomites) for helicopter rescue. Check that your policy covers hiking at altitude — and via ferrata if you'll do any."],
        'it': ["Da sapere: la chiamata d'emergenza (112/118) è gratuita, ma il soccorso in sé — soprattutto in elicottero — qui in Alto Adige può esserti addebitato; per i visitatori non è coperto dal servizio sanitario italiano. Quindi è caldamente consigliata un'assicurazione di viaggio che includa il soccorso alpino e l'evacuazione in elicottero. L'iscrizione a un club alpino (CAI/AVS/DAV/ÖAV) include la copertura per il soccorso, ed esiste anche un programma stagionale a basso costo (Aiut Alpin Dolomites) per il soccorso in elicottero. Verifica che la polizza copra l'escursionismo in quota — e le vie ferrate, se ne farai."],
        'de': ["Gut zu wissen: der Notruf (112/118) ist kostenlos, aber die Rettung selbst — vor allem per Hubschrauber — kann dir hier in Südtirol in Rechnung gestellt werden; für Gäste übernimmt sie der italienische Gesundheitsdienst nicht. Daher ist eine Reiseversicherung mit Bergrettung und Hubschrauberbergung dringend zu empfehlen. Eine Alpenvereins-Mitgliedschaft (CAI/AVS/DAV/ÖAV) enthält den Rettungsschutz, und es gibt zudem ein günstiges Saisonangebot (Aiut Alpin Dolomites) für die Hubschrauberrettung. Prüfe, ob deine Police Wandern in der Höhe abdeckt — und Klettersteige, falls du welche gehst."],
    },
}
